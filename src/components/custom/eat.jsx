...
const Eat = ({ data, setData, translate }) => {
    ...
    return <GridComponent container={true} spacing={2}>
        <GridComponent item={true} size={{ xs: 12 }}>
            <ButtonComponent color={data.type === 1 ? "secondary" : "primary"} onClick={() => {
                handleInputChange('side', null, data, setData);
                handleInputChange('end_date', null, data, setData);
                selectItem(1, "type", data, setData);
            }}>{translate("eat-bottle")}</ButtonComponent>
            <ButtonComponent color={data.type === 2 ? "secondary" : "primary"} onClick={() => {
                handleInputChange('quantity', null, data, setData);
                selectItem(2, "type", data, setData);
            }}>{translate("eat-bosom")}</ButtonComponent>
        </GridComponent>
        <GridComponent item={true} size={{ xs: 12 }}>
        ...
        {
            data.type === 2 ? <GridComponent item={true} size={{ xs: 12 }}>
                <DateTimePickerComponent
        ...
        {
            data.type === 1 ? <GridComponent item={true} size={{ xs: 12 }}>
                <TextFieldComponent
                    ...
                    type={"number"}
                    fullWidth={true} />
            </GridComponent> : <GridComponent item={true} size={{ xs: 12 }}>
                <ButtonComponent color={data.side === 1 ? "secondary" : "primary"} onClick={() => { selectItem(1, "side", data, setData) }}>{translate("left")}</ButtonComponent>
                <ButtonComponent color={data.side === 2 ? "secondary" : "primary"} onClick={() => { selectItem(2, "side", data, setData) }}>{translate("right")}</ButtonComponent>
                <ButtonComponent color={data.side === 3 ? "secondary" : "primary"} onClick={() => { selectItem(3, "side", data, setData) }}>{translate("both")}</ButtonComponent>
            </GridComponent>
        }
        <GridComponent item={true} size={{ xs: 12 }}>
            <TextFieldComponent
